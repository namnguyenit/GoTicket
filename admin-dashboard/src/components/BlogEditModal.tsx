// admin-dashboard/src/components/BlogEditModal.tsx
...
const BlogEditModal: React.FC<BlogEditModalProps> = ({ blogId, onClose }) => {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        // _method: 'PUT' sẽ được thêm tự động trong hàm updateBlog

        try {
            await updateBlog(blogId, formData);
            onClose(true); // Đóng modal và báo hiệu cần tải lại list
        } catch (err: any) {
            console.error("Lỗi khi cập nhật Blog:", err.response?.data);
            const backendErrors = err.response?.data?.errors;
            let errorMessage = err.response?.data?.message || "Cập nhật bài viết thất bại.";
                        ...
                        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                            ...
                            <div>
                                <label className="text-xs font-medium text-gray-500 flex items-center gap-1 mb-1">
                                    <ImageIcon className="h-4 w-4" /> Ảnh bìa (Chọn ảnh mới để thay thế)
                                </label>
                                {imagePreview ? (
                                    <div className="mb-2">
...
// Component helper (sao chép từ BlogCreateModal nếu chưa tách file)
